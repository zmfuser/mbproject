<template>
  <div class="components-container">
    <div class="suifang app-container" ref="box">
      <el-form class="addDdoc" :model="editData" :rules="rules">
        <el-form-item label="标题：" class="title" prop="title">
          <el-input placeholder="请输入标题" :required="true" v-model="editData.title"></el-input>
        </el-form-item>
        ...
        <div style="padding:20px 0; ">
          ...
          <el-form-item label="所属分类：" prop="articleType">
            <el-select v-model="editData.articleType" placeholder="请选择">
              <el-option
        ...
        <div style="padding:20px 0; ">
          ...
          <el-form-item label="推送人群：" prop="pushCrowd">
            <el-select v-model="editData.pushCrowd" placeholder="请选择">
              <el-option
        ...
        <div style="text-align:center;margin-top:10px;">
          ...
          <el-button size="medium" type="warning" @click="saveDraft">保存</el-button>
...
export default {
  ...
  methods: {
    ...
    saveDraft() {
      ...
      _this.editData.articleContent = _this.articleContent;
      _this.editData.dataStatus = 1;
      _this.editData.titleImage = _this.imageUrl;
      saveOrUpdate(_this.editData).then(res => {
        if (res.code == 0) {
  ...
  mounted() {
    let _this = this;
    _this.editData = JSON.parse(sessionStorage.getItem("editData"));
    console.log(_this.editData);
    for (var item of _this.optionsArticleType) {
      if (_this.editData.articleType == item.value) {
        _this.editData.articleType = item.value;
      }
    }
    for (var item of _this.optionsPushCrowd) {
      if (_this.editData.pushCrowd == item.value) {
        _this.editData.pushCrowd = item.value;
      }
    }
    _this.imageUrl = _this.editData.titleImage;
    _this.$bus.emit("setContent", _this.editData);
  }
};
</script>
