...
let netCall = {
	...
	initEvent() {
		...
		window.netcall.on('joinChannel', function (obj) {
			_this.netcall.callAccepted = true
	...
	callingMany() {
		...
		window.netcall
			...
			.then(function (obj) {
				...
				_this.talkVue.closeUI("nc", "open");
				_this.talkVue.closeUI("nc_memberList", "open"); //打开视频框
				_this.createMembergroup(obj, function () {
					(function () {
						var content = {
							type: "type",
							value: "value"
						...
						var Roomaccount = [];
						var list = JSON.parse(_this.talkVue.teamSroom.teamAll);
						var name = _this.talkVue.teamSroom.name;
						...
						var tipname = _this.talkVue.getSign.name;
	...
	calling(talker, item, type, callingType) {
		...
		if (type == "team") {
			...
			_this.account = _this.talkVue.talker.account
		}
	...
	LeaveVideo() {
		...
		_this.talkVue.closeUI("notAnswer1", "close")
		_this.talkVue.closeUI("notAnswer2", "close")
	},
}
