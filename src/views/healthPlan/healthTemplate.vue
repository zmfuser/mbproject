...
export default {
  ...
  methods: {
    // 新增健康计划模板
    saveTempelate(dataStatus) {
      let _this = this;
      _this.form.dataStatus = dataStatus;
      if (dataStatus == 0) {
        this.$confirm("此操作将保存信息且不可更改,是否继续?", "删除确认", {
          confirmButtonText: "继续",
          cancelButtonText: "取消",
          type: "warning"
        })
          .then(() => {
            if (_this.editHealthTemplate) {
              _this.form.code = _this.editHealthTemplate.code;
              saveOrUpdate(_this.form).then(res => {
                if (res.code == 0) {
                  ...
                  sessionStorage.setItem("editHealthTemplate","")
                  _this.$router.push("/healthPlan");
                }
              });
            } else {
              saveOrUpdate(_this.form).then(res => {
