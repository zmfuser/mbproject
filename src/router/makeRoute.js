...
export function generateRoutes(userPermission,state) {
  ...
  var constantMap=constantRouterMap.concat(accessedRouters);
  Vue.set(state,'route',constantMap);
 ...
 * @param route
 */
function hasPermissionRoute(menus, route) {
  if (route.menu) {
    /*
...
function filterAsyncRouter(asyncRouterMap, menus) {
  const accessedRouters = asyncRouterMap.filter(route => {
    ...
    if (hasPermissionRoute(menus, route)) {
